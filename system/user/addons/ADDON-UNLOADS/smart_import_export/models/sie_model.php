...
* @filesource   ./system/expressionengine/third_party/smart_import_export/models/sie_model.php
*
*/
...
class Sie_model extends CI_Model
{
    ...
    **/
    function getExportList($offset, $group_id, $perPage)
    {
        ...
        if($offset === "")
        {
            return $this->db->get('sie_exports')->num_rows;
        }
        else
        {
            
            $data = $this->db->get('sie_exports', $perPage, $offset);
    ...
    **/
    ...
    {
        $this->db->insert('sie_exports', $data);
    ...
    **/
    ...
    {
        $this->db->where('token', $token);
        $this->db->update('sie_exports', $data);
    }
    ...
    **/
    ...
    {
        $data = $this->db->get('sie_exports_settings')->result_array();
        $data = $data[0];
    ...
    **/
    function saveGeneralSettings($data)
    {
        ...
        ee()->db->update('sie_exports_settings', $data);
    }
    ...
    **/
    ...
    {
        $get = ee()->db->select('token')->where($identifier, $value)->get('sie_exports');
        if($get->num_rows > 0)
        {
    ...
    function saveImport(){
        if($_POST['import'] == ''){
            $data = array(
                ...
                'name' => $_POST['import_settings']['import_name'],
                'created_date'  => ee()->localize->now,
                'last_modified' => ee()->localize->now,
                'import_counts' => 0,
                'token'         => strtolower(ee()->functions->random('md5',10)),
                'import_without_login' => 'y',
                // 'type'          => $_POST['import_settings']['import_procedure'],
                'format'        => $_SESSION['Smart_import_export']['setting']['import_file_type'],
                'settings'       => base64_encode(serialize($_SESSION['Smart_import_export'])),
                'status' => 'y',
            );
            $this->db->insert('sie_imports', $data);
            unset($_SESSION['Smart_import_export']);
        }elseif($_POST['import'] > 0){
            $importData = $this->getImportData($_POST['import'])[0];
            $importDataSettingsFull = unserialize(base64_decode($importData['settings']));
            $importDataSettingsFull['setting'] = $_POST['setting'];
            $importDataSettingsFull['configure'] = $_POST;
            $data = array(
                'member_id' => ee()->session->userdata('member_id'),
                'name' => $_POST['import_settings']['import_name'],
                // 'created_date'  => ee()->localize->now,
                ...
                'import_without_login' => 'y',
                // 'type'          => $_POST['import_settings']['import_procedure'],
                'format'        => $_POST['setting']['import_file_type'],
                'settings'       => base64_encode(serialize($importDataSettingsFull)),
                'status' => 'y',
            );
            $this->db->where('id', $_POST['import']);
            $this->db->update('sie_imports', $data);
            unset($_SESSION['Smart_import_export']);
        }
    }
    ...
    **/
    function getImportList($offset, $group_id, $perPage)
    {
        ...
        if($offset === "")
        {
            return $this->db->get('sie_imports')->num_rows;
        }
        else
        {
            
            $data = $this->db->get('sie_imports', $perPage, $offset);
    ...
    /**
    * Get impport settings
    * @param $import_id 
    **/
    function getImportData($import_id){
        $this->db->select('*');
        // $this->db->from('sie_imports');
        $this->db->where('status', 'y');
        $this->db->where('id', $import_id);
        $data = $this->db->get('sie_imports');
        return $data->result_array();
    ...
    * @param $filedir
    **/
    function getFiles($filetype, $filedir){
        $this->db->select('upload_location_id, file_name');
        $this->db->from('files');
        if($filetype != 'all'){
            $this->db->where('mime_type LIKE', '%image%');
        }
        if($filedir != 'all'){
            $this->db->where('upload_location_id', $filedir);
        }
    ...
    /**
    * Get Directory name foe assets folder
    * @param Array of File directory
    **/
    function getDirNameByIDForAssets($dirArr){
        $this->db->distinct();
        $this->db->select('upload_prefs.*,assets_folders.folder_id as assets_folder_id');
        $this->db->from('assets_folders');
        $this->db->join('upload_prefs', 'assets_folders.filedir_id = upload_prefs.id');
        //$this->db->where('site_id', $this->site_id);
        if($dirArr != 'all'){
            foreach($dirArr as $key=>$dir){
              $newDir = str_replace('ee:', '',$dir);
              $dirArr[$key] = $newDir;
            }
            //$dir = explode(':',$dirArr[0]);
            $this->db->where_in('assets_folders.filedir_id', $dirArr);
        }
        $get = $this->db->get();
    ...
    **/
    function deleteImports(){
        foreach($_POST['selection'] as $impID){
            $this->db->where_in('id', $impID);
            $this->db->delete('sie_imports');
        }
        return true;
    }

    /**
    * Save impotr settings
    * @param ImportID
    **/
    function saveImportForm($importID){
        $importData = $this->getImportData($_POST['import'])[0];
        $importDataSettingsFull = unserialize(base64_decode($importData['settings']));
        $importDataSettingsFull['setting'] = $_POST;

        $data = array(
            'member_id' => ee()->session->userdata('member_id'),
            'last_modified' => ee()->localize->now,
            'format'        => $_POST['import_file_type'],
            'settings'       => base64_encode(serialize($importDataSettingsFull)),
        );
        $this->db->where('id', $importID);
        $this->db->update('sie_imports', $data);
        @session_start();
        unset($_SESSION['Smart_import_export']);
    }
    ...
    /**
    * Cron Feature: Get impport settings by token 
    * @param $token
    **/
    function getImportDataByToken($token){
        $this->db->select('*');
        $this->db->where('status', 'y');
        $this->db->where('token', $token);
        $data = $this->db->get('sie_imports');
