...
class Url
{
	...
	public static function cpPublishEditUrl()
	{
		return ee('CP/URL', 'publish/edit')->compile();
	}
	...
	public static function cpEditEntryUrl($entry = array())
	{
		...
		else
		{
			if(version_compare(APP_VER, '4.0.0', '>='))
			{
				if(is_array($entry))
				{
					return ee('CP/URL', 'publish/edit/entry/'.$entry['entry_id'])->compile();
				}
				else
				{
					return ee('CP/URL', 'publish/edit/entry/'.$entry->entry_id)->compile();
	...
	public static function frontUrl($remainder = '')
	{
		ee()->load->helper('url');
