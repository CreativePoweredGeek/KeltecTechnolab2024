...
$lang = [
    ...
    // Cookie Consent
    'ct.cookie.desc.cartthrob_session_id' => 'References the user\'s cart and all the data it contains',
    'ct.cookie.desc.language' => 'Stores the user\'s language preference',
    // ----------------------------------------
    ...
    'coupon_no_access_msg' => 'Your account is not authorized to use this code',
    // Coupon is only available to a specific user group
    ...
    'validation_settings_failed' => 'Error Updating Settings!',
    ...
    'powered_by_title' => 'Powered By CartThrob Shopping Cart System for ExpressionEngine',
    ...
    'save_preset_confirm' => 'Are you sure you want to save this preset?',
    'order_items_field_must_not_be_named_items' => 'The short_name of this field may not be "items". Please choose a different name.',
    ...
    'cartthrob_price_by_member_group_member_group' => 'Role',
    ...
    'nav_exempt_discount_from_tax' => 'Exempt Discount from Tax',
    ...
    'number_format_defaults_prefix' => 'Number Prefix',
    'number_format_defaults_prefix_desc' => 'e.g. $ or &pound;',
    ...
    'global_settings_garbage_collection_cron_description' => 'On high-traffic sites, the built-in session garbage collection (removing old sessions) can intermittently hinder front-end performance. You can work around this by setting this option to yes, which will disable the front-end from triggering garbage collection. You will then be resonsible for setting up a cron job to manage the garbage collection. This is your cron job url:<br><br><code>%1$s</code><br><br>For instance, using curl you could set up your cron job like this: <br><br><code>curl --silent -L "%1$s" 2>&1 /dev/null</code>',
    ...
    'orders_async_method' => 'Asynchronous Order Processing',
    'orders_async_method_description' => '<strong>This feature currently only works with offline payments.</strong><br /><br />On high-traffic sites, order processing can intermittently hinder front-end performance. You can work around this by choosing one of the following methods to process completed orders asynchronously.<br /><br />Choose ‘HTTP’ to process one queued job on each incoming web request.<br /><br />Choose ‘Cron’ if you can create a scheduled cron job on your server. With this option, you can specify the number of queued jobs to process at a time in the URL. This is your cron job url:<br><br><code>%1$s</code><br><br>For instance, using curl you could set up your cron job like this: <br><br><code>curl --silent -L "%1$s" 2>&1 /dev/null</code>',
    'orders_async_method_http' => 'HTTP',
    'orders_async_method_cron' => 'Cron',
    'orders_async_worker_base_url' => 'Asynchronous Worker URL',
    'orders_async_worker_base_url_description' => 'Leave blank unless you have a second worker instance to process orders asynchronously. If you do have a worker, enter the base URL here, including a custom port if any. You do not need to add the action ID in the query string — this will be done automatically.',
    ...
    'rounding_standard' => '<strong>Standard</strong> - Decimals beyond the set precision are rounded naturally',
    'round_up' => '<strong>Round Up</strong> - Decimals beyond the set precision provided are rounded up',
    'round_down' => '<strong>Round Down</strong> - Decimals beyond the set precision provided are rounded down',
    'round_nearest' => '<strong>Round to Nearest Value</strong> - Decimals beyond the set precision are rounded to the nearest value provided below',
    'rounding_nearest_value' => 'Nearest Value (ex: "0.10" rounds to "0.10", "0.20", "0.30", etc.)',

    'global_settings_allow_fractional_quantities' => 'Allow fractional quantities?',
    'msm_show_all' => 'Multi-Site Manager: Show all Order/Product Channels from MSM sites',
    ...
    'orders_card_type_instructions' => 'The Card type (brand) if available.',
    ...
    'discount_limit_by_member_group' => 'Limit by Role',
    'discount_limit_by_member_group_note' => 'Which roles can use this? Enter a list of list of group_id\'s, separated by comma. Leave blank for no limit.',
    ...
    'exempt_discount_from_tax' => 'Exempt Discount from Tax',
    'exempt_discount_from_tax_note' => 'Is the discount taxable?',
...
//	'product_allow_duplicate_instructions' => 'Normally, if "add to cart" is used multiple times on the same product, the quantity of that product in the cart will be increased. If your product can be configured with unique options (like color, shape, size, personalization) set this to "yes" so that multiple different versions will appear in the cart. If this is set to "yes" each time "add to cart" is used on an individual product a completely new product will be added to the cart.',
    ...
    'quebec_descriptive_name' => 'Quebec Tax Descriptive Name (Descriptive name of this tax shown to customer for Quebec)',
    'quebec_effective_rate' => 'Quebec Tax Effective Rate (Effective tax rate shown to customer for Quebec)',
    ...
    'tax_by_class' => 'Tax By Class',
    'tax_by_class_note' => 'This requires that items added to the cart include a tax_class set as an item option, otherwise a GLOBAL tax_class will be used if set (in the Country field).',
    'tax_class' => 'Tax Class',
    ...
    'error_member_group_exists' => 'The role <em>%s</em> already exists and could not be installed',
    ...
    'installed_member_group' => 'Successfully installed the role <em>%s</em>',
    ...
    // G
    'gateway_charge_token_not_supported' => 'The payment gateway currently selected cannot charge tokens. Please contact the site administrator.',
    'gateway_refund_not_supported' => 'The payment gateway currently selected cannot process refunds. Please contact the site administrator.',
    ...
    'single_flat_rate' => 'Single flat rate for the entire purchase',
    ...
    'ct.payments.gateway.offline.note' => 'Allows you take payments manually and without an external payment gateway.',
    'ct.payments.gateway_selection' => 'Primary Gateway',
    'ct.payments.gateway_description' => 'Which Payment Gateway do you wish to use by default (if one isn\'t set)?',
];
