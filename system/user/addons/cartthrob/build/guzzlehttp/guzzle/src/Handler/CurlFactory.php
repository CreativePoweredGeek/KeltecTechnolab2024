...
namespace CartThrob\Dependency\GuzzleHttp\Handler;

use CartThrob\Dependency\GuzzleHttp\Exception\ConnectException;
use CartThrob\Dependency\GuzzleHttp\Exception\RequestException;
use CartThrob\Dependency\GuzzleHttp\Promise as P;
use CartThrob\Dependency\GuzzleHttp\Promise\FulfilledPromise;
use CartThrob\Dependency\GuzzleHttp\Promise\PromiseInterface;
use CartThrob\Dependency\GuzzleHttp\Psr7\LazyOpenStream;
use CartThrob\Dependency\GuzzleHttp\TransferStats;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf) : void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \CartThrob\Dependency\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \CartThrob\Dependency\GuzzleHttp\Psr7\Utils::streamFor($sink);
