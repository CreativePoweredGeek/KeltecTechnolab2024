 ...
 */
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Transaction\AuthOnly;
use CartThrob\Dependency\Academe\AuthorizeNet\Amount\MoneyPhp;
use CartThrob\Dependency\Academe\AuthorizeNet\Amount\Amount;
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Model\NameAddress;
use CartThrob\Dependency\Academe\AuthorizeNet\Payment\CreditCard;
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Model\Customer;
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Model\Retail;
...
use CartThrob\Dependency\Academe\AuthorizeNet\Payment\Track1;
use CartThrob\Dependency\Academe\AuthorizeNet\Payment\Track2;
use CartThrob\Dependency\Academe\AuthorizeNet\Payment\OpaqueData;
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Collections\LineItems;
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Model\LineItem;
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Model\CardholderAuthentication;
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Collections\UserFields;
use CartThrob\Dependency\Academe\AuthorizeNet\Request\Model\UserField;
use CartThrob\Dependency\Money\Parser\DecimalMoneyParser;
use CartThrob\Dependency\Money\Currencies\ISOCurrencies;
...
class AuthorizeRequest extends AbstractRequest
{
    ...
    public function getData()
    {
        ...
        $customer = $customer->withId($this->getCustomerId())->withCustomerType($this->getCustomerType())->withDriversLicense($this->getCustomerDriversLicense())->withTaxId($this->getCustomerTaxId());
        if ($card = $this->getCard()) {
            $billingAddress = \trim($card->getBillingAddress1() . ' ' . $card->getBillingAddress2());
            if ($billingAddress === '') {
                $billingAddress = null;
            }
            $billTo = new NameAddress($card->getBillingFirstName(), $card->getBillingLastName(), $card->getBillingCompany(), $billingAddress, $card->getBillingCity(), $card->getBillingState(), $card->getBillingPostcode(), $card->getBillingCountry());
            ...
            $shippingAddress = \trim($card->getShippingAddress1() . ' ' . $card->getShippingAddress2());
            if ($shippingAddress === '') {
                $shippingAddress = null;
            }
            $shipTo = new NameAddress($card->getShippingFirstName(), $card->getShippingLastName(), $card->getShippingCompany(), $shippingAddress, $card->getShippingCity(), $card->getShippingState(), $card->getShippingPostcode(), $card->getShippingCountry());
            if ($shipTo->hasAny()) {
            ...
            if ($card->getNumber()) {
                ...
                $transaction = $transaction->withPayment(new Track1($card->getTrack1()));
            } elseif ($card->getTrack2()) {
                $transaction = $transaction->withPayment(new Track2($card->getTrack2()));
            }
        }
        // credit card
        if ($customer->hasAny()) {
        ...
        if ($descriptor && $value) {
            $transaction = $transaction->withPayment(new OpaqueData($descriptor, $value));
        }
        if ($this->getClientIp()) {
        ...
        if ($this->getDeviceType() || $this->getMarketType() || isset($card) && $card->getTracks()) {
            ...
            $retail = new Retail($this->getMarketType() ?: Retail::MARKET_TYPE_RETAIL, $this->getDeviceType() ?: Retail::DEVICE_TYPE_UNKNOWN);
        ...
        if ($this->getAuthenticationIndicator() || $this->getAuthenticationValue()) {
            $cardholderAuthentication = new CardholderAuthentication($this->getAuthenticationIndicator(), $this->getAuthenticationValue());
        ...
        $transaction = $transaction->with(['terminalNumber' => $this->getTerminalNumber()]);
