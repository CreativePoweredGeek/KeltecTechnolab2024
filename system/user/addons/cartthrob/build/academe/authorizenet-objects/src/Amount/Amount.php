...
namespace CartThrob\Dependency\Academe\AuthorizeNet\Amount;
...
class Amount extends AbstractModel implements AmountInterface
{
    ...
    protected function setMajorUnit($amount)
    {
        if (\is_int($amount) || \is_float($amount) || \is_string($amount) && \preg_match('/^[0-9]*\\.[0-9]*$/', $amount)) {
            ...
            if (\floor($amount) != \round($amount, 5)) {
                ...
                throw new UnexpectedValueException(\sprintf('Amount has too many decimal places. Calculated minor unit %f should be an integer.', $amount));
            }
            $this->setMinorUnit((int) $amount);
        } else {
            throw new UnexpectedValueException(\sprintf('Major unit is not a number.'));
        }
    }
    /**
     * Set the minot unit.
    ...
    protected function setMinorUnit($amount)
    {
        if (\is_int($amount) || \is_string($amount) && \preg_match('/^[0-9]+$/', $amount)) {
            ...
            throw new UnexpectedValueException(\sprintf('Minor unit is not an integer.'));
        }
    }
    /**
    ...
    public static function __callStatic($name, array $arguments)
    {
        try {
            ...
            throw new Exception(\sprintf('Call to undefined method $class::%s() in %s on line %d', \get_called_class(), $trace[0]['file'], $trace[0]['line']));
        }
    ...
    {
        // FIXME!!!
        return \number_format($this->amount / \pow(10, $this->getDecimals()), $this->getDecimals(), '.', '');
    }
