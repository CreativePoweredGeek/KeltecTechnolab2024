...
class BankAccount extends AbstractModel implements PaymentInterface
{
    ...
    public function jsonSerialize()
    {
        ...
        if ($this->hasEcheckType()) {
            ...
            if ($checkNumberRequired && !$this->hasCheckNumber()) {
                ...
                throw new \InvalidArgumentException(\sprintf('The checkNumber is required when the echeckType is "%s"; the checkNumber is not set', $getEcheckType));
            }
    ...
    {
        return \str_repeat($this->masking_character, \strlen($this->getAccountNumber()) - 4) . \substr($this->getAccountNumber(), -4);
    }
    /**
    ...
    {
        return \str_repeat($this->masking_character, \strlen($this->getRoutingNumber()) - 4) . \substr($this->getRoutingNumber(), -4);
    }
