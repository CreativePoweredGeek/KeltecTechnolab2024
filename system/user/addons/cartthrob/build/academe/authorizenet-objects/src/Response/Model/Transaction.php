 ...
 * [ ] profile
 */
use CartThrob\Dependency\Academe\AuthorizeNet\Response\Collections\LineItems;
use CartThrob\Dependency\Academe\AuthorizeNet\Amount\Simple as SimpleAmount;
    ...
    public function __construct($data)
    {
        ...
        if ($lineItemsData = $this->getDataValue('lineItems')) {
            ...
            foreach ($lineItemsData as $key => $lineItemData) {
                $lineItem = new LineItem($this->getDataValue("lineItems.{$key}.itemId"), $this->getDataValue("lineItems.{$key}.name"), $this->getDataValue("lineItems.{$key}.description"), $this->getDataValue("lineItems.{$key}.quantity"), new SimpleAmount($this->getDataValue("lineItems.{$key}.unitPrice")), $this->getDataValue("lineItems.{$key}.taxable"));
