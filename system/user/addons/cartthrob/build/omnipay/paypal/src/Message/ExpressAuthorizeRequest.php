...
use CartThrob\Dependency\Omnipay\PayPal\Support\InstantUpdateApi\ShippingOption;
use CartThrob\Dependency\Omnipay\PayPal\Support\InstantUpdateApi\BillingAgreement;
/**
        ...
        if (!empty($callback)) {
            ...
            if (!empty($shippingOptions)) {
                foreach ($shippingOptions as $index => $shipping) {
                    ...
                    $data['L_SHIPPINGOPTIONAMOUNT' . $index] = \number_format($shipping->getAmount(), 2);
