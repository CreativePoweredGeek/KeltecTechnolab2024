 ...
 */
abstract class AbstractRestRequest extends \CartThrob\Dependency\Omnipay\Common\Message\AbstractRequest
{
    ...
    public function sendData($data)
    {
        ...
        try {
            $httpResponse = $this->httpClient->request($this->getHttpMethod(), $this->getEndpoint(), array('Accept' => 'application/json', 'Authorization' => 'Bearer ' . $this->getToken(), 'Content-type' => 'application/json'), $body);
            // Empty response body should be parsed also as and empty array
            $body = (string) $httpResponse->getBody()->getContents();
            $jsonToArrayResponse = !empty($body) ? \json_decode($body, \true) : array();
    ...
    public function toJSON($data, $options = 0)
    {
        ...
        if (\version_compare(\phpversion(), '5.4.0', '>=') === \true) {
