...
use CartThrob\Dependency\Omnipay\Mollie\Message\Response\FetchPaymentMethodsResponse;
/**
...
class FetchPaymentMethodsRequest extends AbstractMollieRequest
{
    ...
    public function getData()
    {
        ...
        if ($this->getAmount() || $this->getCurrency()) {
            ...
            $amount = ['value' => $this->getAmount(), 'currency' => $this->getCurrency()];
        }
        return ['amount' => $amount, 'billingCountry' => $this->getBillingCountry(), 'locale' => $this->getLocale(), 'resource' => $this->getResource(), 'includeWallets' => $this->getIncludeWallets(), 'sequenceType' => $this->getSequenceType()];
    }
    /**
     * @param array $data
