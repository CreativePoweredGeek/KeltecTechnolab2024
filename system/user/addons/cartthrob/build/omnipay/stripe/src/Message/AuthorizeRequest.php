...
use CartThrob\Dependency\Omnipay\Stripe\StripeItem;
use CartThrob\Dependency\Omnipay\Stripe\StripeItemBag;
/**
 * Stripe Authorize Request.
 *
        ...
        if ($items = $this->getItems()) {
            ...
            if ($this->validateLineItemsForLevel3($items)) {
                ...
                foreach ($items as $item) {
                    $lineItem = ['product_code' => \substr($item->getName(), 0, 12), 'product_description' => \substr($item->getDescription(), 0, 26)];
                    if ($item->getPrice()) {
                ...
                $data['level3'] = ['merchant_reference' => $this->getTransactionId(), 'line_items' => $lineItems];
            }
        }
