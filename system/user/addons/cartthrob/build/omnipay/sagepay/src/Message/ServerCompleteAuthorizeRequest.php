...
class ServerCompleteAuthorizeRequest extends AbstractRequest
{
    ...
    public function getSignature()
    {
        ...
        $reference = \json_decode($this->getTransactionReference(), \true);
        // Re-create the VPSSignature
        $signature_string = $reference['VPSTxId'] . $reference['VendorTxCode'] . $this->httpRequest->request->get('Status') . $this->httpRequest->request->get('TxAuthNo', '') . $this->getVendor() . $this->httpRequest->request->get('AVSCV2', '') . $reference['SecurityKey'] . $this->httpRequest->request->get('AddressResult', '') . $this->httpRequest->request->get('PostCodeResult', '') . $this->httpRequest->request->get('CV2Result', '') . $this->httpRequest->request->get('GiftAid', '') . $this->httpRequest->request->get('3DSecureStatus', '') . $this->httpRequest->request->get('CAVV', '') . $this->httpRequest->request->get('AddressStatus') . $this->httpRequest->request->get('PayerStatus', '') . $this->httpRequest->request->get('CardType', '') . $this->httpRequest->request->get('Last4Digits', '') . $this->httpRequest->request->get('DeclineCode', '') . $this->httpRequest->request->get('ExpiryDate', '') . $this->httpRequest->request->get('FraudResponse', '') . $this->httpRequest->request->get('BankAuthCode', '');
