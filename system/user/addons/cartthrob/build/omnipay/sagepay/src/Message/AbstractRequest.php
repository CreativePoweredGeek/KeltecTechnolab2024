...
use CartThrob\Dependency\Omnipay\SagePay\Extend\Item as ExtendItem;
use CartThrob\Dependency\Omnipay\Common\Message\AbstractRequest as OmnipayAbstractRequest;
use CartThrob\Dependency\Omnipay\SagePay\Traits\GatewayParamsTrait;
use CartThrob\Dependency\Omnipay\SagePay\ConstantsInterface;
...
abstract class AbstractRequest extends OmnipayAbstractRequest implements ConstantsInterface
{
    ...
    protected function getAddressData($type = 'Billing')
    {
        ...
        $mapping = ['Firstnames' => 'FirstName', 'Surname' => 'LastName', 'Address1' => 'Address1', 'Address2' => 'Address2', 'City' => 'City', 'PostCode' => 'Postcode', 'State' => 'State', 'Country' => 'Country', 'Phone' => 'Phone'];
        $data = [];
    ...
    {
        $address = $this->getAddressData((bool) $this->getBillingForShipping() ? 'Billing' : 'Shipping');
        foreach ($address as $name => $value) {
    ...
    public static function parseBodyData(ResponseInterface $httpResponse)
    {
        ...
        foreach ($lines as $line) {
            $line = \explode('=', $line, 2);
            if (!empty($line[0])) {
                $responseData[\trim($line[0])] = isset($line[1]) ? \trim($line[1]) : '';
            }
        }
        return $responseData;
    }
    ...
    {
        return \sprintf('%s/%s.vsp', $this->getTestMode() ? $this->testEndpoint : $this->liveEndpoint, $this->getService());
    }
    /**
    ...
    protected function filterItemName($name)
    {
        ...
        $name = \trim(\substr(\preg_replace($pattern, '', $name), 0, 100));
        return $name;
    }
    /**
    ...
    protected function getItemDataNonXML()
    {
        ...
        foreach ($items as $basketItem) {
            ...
            $result .= ':' . $description . ':' . $basketItem->getQuantity() . ':' . \number_format($basketItem->getPrice(), 2, '.', '') . ':' . $vat . ':' . \number_format($basketItem->getPrice() + $vat, 2, '.', '') . ':' . \number_format($lineTotal, 2, '.', '');
            // Line total
            $count++;
        }
    ...
    {
        $reference = \json_decode($value, \true);
