...
use CartThrob\Dependency\Omnipay\SagePay\Traits\ResponseFieldsTrait;
...
class Response extends AbstractResponse implements RedirectResponseInterface, ConstantsInterface
{
    ...
    {
        return $this->getStatus() === static::SAGEPAY_STATUS_OK || $this->getStatus() === static::SAGEPAY_STATUS_OK_REPEATED || $this->getStatus() === static::SAGEPAY_STATUS_REGISTERED || $this->getStatus() === static::SAGEPAY_STATUS_AUTHENTICATED;
    }
    /**
     * Gateway Reference
     *
    ...
    {
        if ($this->isRedirect()) {
            return array('PaReq' => $this->getDataItem('PAReq'), 'TermUrl' => $this->getRequest()->getReturnUrl(), 'MD' => $this->getDataItem('MD'));
        }
    }
    /**
