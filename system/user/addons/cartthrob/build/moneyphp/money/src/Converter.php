...
final class Converter
{
    ...
    {
        return $this->convertAgainstCurrencyPair($money, $this->exchange->quote($money->getCurrency(), $counterCurrency), $roundingMode);
    }
    ...
    public function convertAgainstCurrencyPair(Money $money, CurrencyPair $currencyPair, int $roundingMode = Money::ROUND_HALF_UP) : Money
    {
        if (!$money->getCurrency()->equals($currencyPair->getBaseCurrency())) {
            throw new InvalidArgumentException(sprintf('Expecting to convert against base currency %s, but got %s instead', $money->getCurrency()->getCode(), $currencyPair->getBaseCurrency()->getCode()));
        }
