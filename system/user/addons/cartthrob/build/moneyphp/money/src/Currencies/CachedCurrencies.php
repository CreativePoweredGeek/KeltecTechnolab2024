...
use ArrayIterator;
use CartThrob\Dependency\Cache\TagInterop\TaggableCacheItemInterface;
...
final class CachedCurrencies implements Currencies
{
    ...
    {
        return new CallbackFilterIterator(new ArrayIterator(iterator_to_array($this->currencies->getIterator())), function (Currency $currency) : bool {
            $item = $this->pool->getItem('currency|availability|' . $currency->getCode());
            $item->set(\true);
            if ($item instanceof TaggableCacheItemInterface) {
                $item->setTags(['currency.availability']);
