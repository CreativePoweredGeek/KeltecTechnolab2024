 ...
 */
namespace CartThrob\Dependency\Symfony\Component\PropertyAccess;

use CartThrob\Dependency\Psr\Cache\CacheItemPoolInterface;
use CartThrob\Dependency\Psr\Log\LoggerInterface;
use CartThrob\Dependency\Psr\Log\NullLogger;
use CartThrob\Dependency\Symfony\Component\Cache\Adapter\AdapterInterface;
use CartThrob\Dependency\Symfony\Component\Cache\Adapter\ApcuAdapter;
use CartThrob\Dependency\Symfony\Component\Cache\Adapter\NullAdapter;
use CartThrob\Dependency\Symfony\Component\Inflector\Inflector;
use CartThrob\Dependency\Symfony\Component\PropertyAccess\Exception\AccessException;
use CartThrob\Dependency\Symfony\Component\PropertyAccess\Exception\InvalidArgumentException;
use CartThrob\Dependency\Symfony\Component\PropertyAccess\Exception\NoSuchIndexException;
use CartThrob\Dependency\Symfony\Component\PropertyAccess\Exception\NoSuchPropertyException;
use CartThrob\Dependency\Symfony\Component\PropertyAccess\Exception\UnexpectedTypeException;
/**
...
class PropertyAccessor implements PropertyAccessorInterface
{
        ...
        try {
            if (self::ACCESS_TYPE_METHOD === $access[self::ACCESS_TYPE]) {
                try {
                    ...
                    if (__FILE__ === $trace['file'] && $access[self::ACCESS_NAME] === $trace['function'] && $object instanceof $trace['class'] && \preg_match('/Return value (?:of .*::\\w+\\(\\) )?must be of (?:the )?type (\\w+), null returned$/', $e->getMessage(), $matches)) {
    ...
    public static function createCache($namespace, $defaultLifetime, $version, LoggerInterface $logger = null)
    {
        if (null === $defaultLifetime) {
            @\trigger_error(\sprintf('Passing null as "$defaultLifetime" 2nd argument of the "%s()" method is deprecated since Symfony 4.4, pass 0 instead.', __METHOD__), \E_USER_DEPRECATED);
        }
