...
use CartThrob\Dependency\DI\Annotation\Inject;
use CartThrob\Dependency\DI\Annotation\Injectable;
use CartThrob\Dependency\DI\Definition\Exception\InvalidAnnotation;
use CartThrob\Dependency\DI\Definition\ObjectDefinition;
use CartThrob\Dependency\DI\Definition\ObjectDefinition\MethodInjection;
use CartThrob\Dependency\DI\Definition\ObjectDefinition\PropertyInjection;
use CartThrob\Dependency\DI\Definition\Reference;
use CartThrob\Dependency\Doctrine\Common\Annotations\AnnotationRegistry;
use CartThrob\Dependency\Doctrine\Common\Annotations\Reader;
use CartThrob\Dependency\Doctrine\Common\Annotations\SimpleAnnotationReader;
use InvalidArgumentException;
use CartThrob\Dependency\PhpDocReader\PhpDocReader;
use ReflectionClass;
...
class AnnotationBasedAutowiring implements DefinitionSource, Autowiring
{
    ...
    private function readProperty(ReflectionProperty $property, ObjectDefinition $definition, $classname = null)
    {
        ...
        $annotation = $this->getAnnotationReader()->getPropertyAnnotation($property, 'CartThrob\\Dependency\\DI\\Annotation\\Inject');
    ...
    private function getMethodInjection(ReflectionMethod $method)
    {
        // Look for @Inject annotation
        try {
            $annotation = $this->getAnnotationReader()->getMethodAnnotation($method, 'CartThrob\\Dependency\\DI\\Annotation\\Inject');
    ...
    public function getAnnotationReader()
    {
        if ($this->annotationReader === null) {
            ...
            $this->annotationReader->addNamespace('CartThrob\\Dependency\\DI\\Annotation');
        }
    ...
    private function readInjectableAnnotation(ReflectionClass $class, ObjectDefinition $definition)
    {
        try {
            ...
            $annotation = $this->getAnnotationReader()->getClassAnnotation($class, 'CartThrob\\Dependency\\DI\\Annotation\\Injectable');
