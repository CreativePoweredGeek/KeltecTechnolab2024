...
var ct_shipping_fields = new   Array('shipping_first_name','shipping_last_name','shipping_phone','shipping_address','shipping_address2','shipping_city','shipping_state','shipping_zip','shipping_country_code','shipping_company','shipping_option');
...
function update_cart(data, statusText, xhr, $form)  {   
    if (data.success) {                                                   
        ...
        jQuery('.total_items_count')		.html( data.total_items );
        jQuery('.unique_items_count')		.html( data.total_unique_items );
...
jQuery(document).ready(function($){
	...
	var form = $('#checkout_form').get(0);
	if(form) {
		form.addEventListener('ct.payment-form.submit', function (e) {
			$('input[type=submit], button[type=submit]', this).attr('disabled', 'disabled').addClass('disabled').val("Submitting...");
		});
		form.addEventListener('ct.payment-form.error', function (e) {
			$('input[type=submit], button[type=submit]', this).removeAttr('disabled').removeClass('disabled').val("Complete Checkout");
		});
	}
