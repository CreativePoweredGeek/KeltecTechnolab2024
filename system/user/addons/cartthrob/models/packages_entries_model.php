...
class Packages_entries_model extends CI_Model
{
     ...
     * @return $this
     */
    public function loadEntriesByEntryId($entry_ids)
    {
        ...
        if ($entry_ids) {
            return $this->load_entries(['channel_titles.entry_id' => $entry_ids]);
        }
    ...
    public function load_entries($data = [])
    {
        ...
        foreach ($query->result_array() as $row) {
            ...
            foreach ($this->packages_field_model->get_fields() as $field) {
    ...
    public function create_entry(array $data)
    {
        ...
        foreach ($fields as $k => $field) {
            ...
            if (isset($data['field_id_' . $field['field_id']])) {
                ...
                $channel_field_data['field_id_' . $field['field_id']] = $field_data;
            } else {
            ...
            if (isset($data['field_ft_' . $field['field_id']])) {
                $channel_field_data['field_ft_' . $field['field_id']] = $data['field_ft_' . $field['field_id']];
            } else {
                $channel_field_data['field_ft_' . $field['field_id']] = $field['field_fmt'];
            }

            $this->db->insert('channel_data_field_' . $field['field_id'], $channel_field_data);

            unset($channel_field_data);
        }
        ...
        $this->db->update('members', ['total_entries' => $total_entries],
            ['member_id' => $data['author_id']]);
    ...
    public function update_entry($entry_id, $data)
    {
        ...
        $fields = ($channel_id) ? $this->packages_field_model->get_fields_by_channel($channel_id) : [];
        ...
        $channel_titles['edit_date'] = $this->localize->now - 60;

        $this->db->update('channel_titles', $channel_titles, ['entry_id' => $entry_id]);
        ...
        foreach ($fields as $field) {
            ...
            if ($channel_field_data) {
                $this->db->update('channel_data_field_' . $field['field_id'], $channel_field_data,
                    ['entry_id' => $entry_id]);
            }
        ...
        if ($channel_data) {
            $this->db->update('channel_data', $channel_data, ['entry_id' => $entry_id]);
        }
