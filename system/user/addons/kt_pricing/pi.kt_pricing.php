...
class KT_pricing
{
	...
	{
		if($pMaxDisc == 0) {  // Added this check on 8/1/2020
			$value = $oprice;
		} else {
			...
			$cdiscount = $item->field_id_213;
				...
				if (($pMaxDisc=="")||($pMaxDisc=="0")) {
				} else {
					if($cdiscount > $pMaxDisc) { $cdiscount = $pMaxDisc; }
				}

				$trimmed_price = (double)str_replace(array(',','$'), '', $oprice);
		...
		if($pMaxDisc == 0) {  // Added this check on 8/1/2020
			...
			if($entries != "") {
				...
				if (($pMaxDisc=="")||($pMaxDisc=="0")) {
					...
					if($bdiscount > $pMaxDisc) { $bdiscount = $pMaxDisc; }
				}
