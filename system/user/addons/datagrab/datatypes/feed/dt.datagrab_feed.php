 ...
 */
class Datagrab_feed extends AbstractDataType
{
    public $type = 'RSS/Atom Feed';
    var $datatype_info = [
        'name' => 'RSS/ATOM feed (Deprecated)',
        'version' => '0.9',
        'description' => '<span class="st-warning">Warning</span> This uses a very old 3rd party RSS library and is deprecated. Please use the The XML datatype instead. This datatype option is only available for legacy imports and is no longer supported.',
        'allow_subloop' => true
    ];
    ...
    public function settings_form(array $values = []): array
    {
        return [
            ...
            [
                ...
                'desc' => 'Only import entries that match these values. Comma-separated keywords. Leave blank to import all. eg, if the objects are returned in a \'data\' element you should specify it in this field.',
                'fields' => [
                    'filter' => [
                        'type' => 'text',
    ...
    function fetch()
    {
        ...
        if (substr($this->getFilename(), 0, 7) == "feed://") {
            $this->settings["filename"] = "http://" . substr($this->getFilename(), 7);
        }

        set_error_handler(array("Datagrab_feed", "_catch_error"));
        $RSS = datagrab_fetch_rss($this->getFilename());
        restore_error_handler();

        if (!is_object($RSS)) {
            $this->addError('Cannot load the feed.');
            return -1;
        }
    ...
    function get_item($items, $id): string
    {
        if (strpos($id, ':')) {
    ...
    function get_sub_item($item, $id, $config, $field, array $column = [])
    {
...
class Datagrab_MagpieRSS
{
                         ...
                         $input_encoding = null, $detect_encoding = true)
    {
        ...
        if (!function_exists('xml_parser_create')) {
            $this->datagrab_error("Failed to load PHP's XML Extension. " .
        ...
        if (!is_resource($parser)) {
            $this->datagrab_error("Failed to create an instance of PHP's XML parser. " .
                "http://www.php.net/manual/en/ref.xml.php",
                E_USER_ERROR);
        }
    ...
    function php4_create_parser($source, $in_enc, $detect)
    {
        ...
        // else
        $this->datagrab_error("Feed is in an unsupported character encoding. ($in_enc) " .
    ...
    function datagrab_error($errormsg, $lvl = E_USER_WARNING)
    {
 ...
 * behaviour of datagrab_fetch_rss() currently, most options effect the cache
 *
...
/*=======================================================================*\
    ...
    NOTES ON CACHEING:
    If caching is on (MAGPIE_CACHE_ON) datagrab_fetch_rss will first check the cache.

    NOTES ON RETRIEVING REMOTE FILES:
    If conditional gets are on (MAGPIE_CONDITIONAL_GET_ON) datagrab_fetch_rss will
...
function datagrab_fetch_rss($url)
{
    ...
    if (!isset($url)) {
        datagrab_error("datagrab_fetch_rss called without a url");
        return false;
    }
    ...
    if (!MAGPIE_CACHE_ON) {
        ...
        $resp = _datagrab_fetch_remote_file($url);
        if (datagrab_is_success($resp->status)) {
            return _datagrab_response_to_rss($resp);
        } else {
            datagrab_error("Failed to fetch $url and cache is off");
            return false;
        }
    } // else cache is ON
    else {
        ...
        $cache = new Datagrab_RSSCache(MAGPIE_CACHE_DIR, MAGPIE_CACHE_AGE);

        if (MAGPIE_DEBUG && $cache->ERROR) {
            datagrab_debug($cache->ERROR, E_USER_WARNING);
        }
        ...
        $cache_key = $url . DATAGRAB_MAGPIE_OUTPUT_ENCODING;
        ...
        if ($cache_status == 'HIT') {
            ...
            if (isset($rss) && $rss) {
                ...
                $rss->from_cache = 1;
                if (MAGPIE_DEBUG > 1) {
                    datagrab_debug("MagpieRSS: Cache HIT", E_USER_NOTICE);
                }
                return $rss;
            }
        }
        ...
        if ($cache_status == 'STALE') {
            $rss = $cache->get($cache_key);
            if ($rss && isset($rss->etag) && $rss->etag && $rss->last_modified) {
                $request_headers['If-None-Match'] = $rss->etag;
                $request_headers['If-Last-Modified'] = $rss->last_modified;
            }
        }

        $resp = _datagrab_fetch_remote_file($url, $request_headers);

        if (isset($resp) and $resp) {
            if ($resp->status == '304') {
                // we have the most current copy
                if (MAGPIE_DEBUG > 1) {
                    datagrab_debug("Got 304 for $url");
                }
                // reset cache on 304 (at minutillo insistent prodding)
                $cache->set($cache_key, $rss);
                return $rss;
            } elseif (datagrab_is_success($resp->status)) {
                $rss = _datagrab_response_to_rss($resp);
                if ($rss) {
                    if (MAGPIE_DEBUG > 1) {
                        datagrab_debug("Fetch successful");
                    }
                    // add object to cache
                    $cache->set($cache_key, $rss);
                    return $rss;
        ...
        if ($rss) {
            if (MAGPIE_DEBUG) {
                datagrab_debug("Returning STALE object for $url");
            }
            return $rss;
        }

        // else we totally failed
        datagrab_error($errormsg);

        return false;

    } // end if ( !MAGPIE_CACHE_ON ) {
} // end datagrab_fetch_rss()
...
function datagrab_debug($debugmsg, $lvl = E_USER_NOTICE)
{
...
\*=======================================================================*/
function datagrab_magpie_error($errormsg = "")
{
    global $MAGPIE_ERROR;
...
/*=======================================================================*\
    Function:   _datagrab_fetch_remote_file
    Purpose:    retrieve an arbitrary remote file
    Input:      url of the remote file
                headers to send along with the request (optional)
    Output:     an HTTP response object (see Snoopy.class.inc)
\*=======================================================================*/
function _datagrab_fetch_remote_file($url, $headers = "")
{
...
/*=======================================================================*\
    Function:   _datagrab_response_to_rss
    Purpose:    parse an HTTP response object into an RSS object
    Input:      an HTTP response object (see Snoopy)
    Output:     parsed RSS object (see rss_parse)
\*=======================================================================*/
function _datagrab_response_to_rss($resp)
{
    $rss = new Datagrab_MagpieRSS($resp->results, DATAGRAB_MAGPIE_OUTPUT_ENCODING, MAGPIE_INPUT_ENCODING, MAGPIE_DETECT_ENCODING);
...
/*=======================================================================*\
    Function:   datagrab_init
    Purpose:    setup constants with default values
                check for user overrides
\*=======================================================================*/
function datagrab_init()
{
    if (defined('DATAGRAB_MAGPIE_INITALIZED')) {
        return;
    } else {
        define('DATAGRAB_MAGPIE_INITALIZED', true);
    }
    ...
    if (!defined('DATAGRAB_MAGPIE_OUTPUT_ENCODING')) {
        define('DATAGRAB_MAGPIE_OUTPUT_ENCODING', 'UTF-8');
    }
...
/*=======================================================================*\
    Function:   datagrab_is_info
    Purpose:    return true if Informational status code
\*=======================================================================*/
function datagrab_is_info($sc)
{
    return $sc >= 100 && $sc < 200;
}

/*=======================================================================*\
    Function:   datagrab_is_success
    Purpose:    return true if Successful status code
\*=======================================================================*/
function datagrab_is_success($sc)
{
...
/*=======================================================================*\
    Function:   datagrab_client_error
    Purpose:    return true if Error status code, and its a client error
\*=======================================================================*/
function datagrab_client_error($sc)
{
...
\*=======================================================================*/
function is_datagrab_server_error($sc)
{
...
class Datagrab_RSSCache
{
    ...
    {
        if (MAGPIE_DEBUG) {
            $this->datagrab_error("MagpieRSS [debug] $debugmsg", $lvl);
        }
    }
